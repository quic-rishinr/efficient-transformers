...
# Use a shorter compiled/inference sequence length than the library default
# (WAV2VEC2_MAX_SEQ_LEN = 480000 ≈ 30s) to cut ONNX-export/QAIC-compile time. 160000
# samples (10s @ 16kHz) still comfortably covers the short LibriSpeech-dummy utterance,
# so the PyTorch/ORT/AI100 transcripts remain identical. The same value is used for the
# PyTorch reference, the ORT run, and the QAIC compile so all backends share one shape.
WAV2VEC2_TEST_SEQ_LEN = 160000
assert WAV2VEC2_TEST_SEQ_LEN <= WAV2VEC2_MAX_SEQ_LEN
